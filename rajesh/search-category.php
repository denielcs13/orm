...
$data2=mysqli_fetch_assoc($sql);
	/* $sub_cat="select * from subcategory1 where cat_id='$data[category_id]'";
												$data_cat2=fetch_query($con,$sub_cat);*/
												foreach($data2 as $data){   
    $product="select * from product where category='".$data['category_id']."'";
								$product_data=fetch_query($con,$product);
									foreach($product_data as $pro_data)
		{ 
		 $query2="select * from review where product_id='".$pro_data['company_id']."'";
