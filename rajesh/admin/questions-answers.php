  ...
  <div class="boxed">
    ...
    <div id="content-container">
      <div class="pageheader hidden-xs">
        <h3><i class="fa fa-file-text-o"></i>Questions-Answers Details </h3>
        <div class="breadcrumb-wrapper"> <span class="label">You are here:</span>
          <ol class="breadcrumb">
            <li> <a href="dashbord.php"> Home </a> </li>
            <li class="active">Questions-Answers Details </li>
          </ol>
        </div>
      </div>
      <!--Page content-->
...
$totalEmpSQL = "select * from question where admin_id='".$admin_id."'";
...
$query = "select * from question  order by q_id desc LIMIT $startFrom, $showRecordPerPage";	
	  				
				//$query="Select * from question where admin_id='".$admin_id."' order by q_id desc";
					  $i=1;					  
                    ...
                    <table id="demo-dt-basic" class="table table-striped table-bordered">
                      ...
                      <tbody  id="project_search2">
						...
						<td> <?php echo $i; ?></td>
                          ...
                          <td><?php echo $about_data['q_name']; ?></td>
						 <?php $query2="select * from answer where ques_id='$about_data[q_id]'";
								$data3=fetch_query($con,$query2);  ?>
						
						   <td><?php foreach($data3 as $newdata){
						 
						  echo  $newdata['answer'].'<br>'; 
						  }
						  ?>
						  </td>
							
 <td align="center"><a href="meta?Q_ID=<?php echo $about_data['q_id']; ?>&&type=question"  class="btn btn-warning"> Add Meta</a> </td>
 

                          <td><?php if($about_data['status']==1) {?>
                            <a class="btn btn-success" id="change_status" data-id="<?php echo $about_data['q_id']; ?>" data-admin_id="<?php echo $about_data['admin_id']; ?>" data-status="<?php echo $about_data['status']; ?>">Active</a>
                            <?php } else {?>
                               <a class="btn btn-danger" id="change_status" data-id="<?php echo $about_data['q_id']; ?>" data-admin_id="<?php echo $about_data['admin_id']; ?>" data-status="<?php echo $about_data['status']; ?>">In-active</a>
                              <?php
							}
						?></td>
                          
                  <td><a class="btn btn-info btn-sm editproject" data-toggle="modal" data-target="#myModal" id="update" data-id="<?php echo $about_data['q_id']; ?>" data-admin_id="<?php echo $about_data['admin_id']; ?>"><span class="fa fa-pencil-square-o"></span> </a> </td>
						  
                      <td><a style="text-decoration:none;" class="approved btn btn-danger" id="delete" data-id="<?php echo $about_data['q_id']; ?>" data-admin_id="<?php echo $about_data['admin_id']; ?>" ><i class="fa fa-trash"></i> </a>
                                                      </td>
                        </tr>
...
$(document).ready(function(){
	 a_url='ajax/questions-answers.php';
	...
	$("body").on("click","#change_status",function(){	   
		   ...
		   var postdata="action=CHANGE_STATUS&change_status="+a_status+"&q_id="+a_id+"&admin_id="+admin_id;
		...
		$("body").on("click","#delete",function(){	   
		   ...
		   var admin_id=$(this).data('admin_id');
		   var error_id=$("#error-msg-page");
		   var postdata="action=DELETE_DATA"+"&q_id="+a_id+"&admin_id="+admin_id;
