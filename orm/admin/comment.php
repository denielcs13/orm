...
$totalEmpSQL = "select * from comment where admin_id='".$admin_id."'";
...
$query = "select * from comment  order by c_id desc LIMIT $startFrom, $showRecordPerPage";
			
		//$query="Select * from comment where admin_id='".$admin_id."' ORDER BY c_id ASC LIMIT $start_from, $limit";
					  $i=1;					  
                    ...
                    <table id="demo-dt-basic" class="table table-striped table-bordered">
                      ...
                      <tbody  id="project_search2">
						...
						<td> <?php echo $i; ?></td>
                          ...
                          <td><?php echo $about_data['c_description']; ?></td>
 <td align="center"><a href="meta?CO_ID=<?php echo $about_data['c_id']; ?>&&type=comment"  class="btn btn-warning"> Add Meta</a> </td>       
                          <td><?php if($about_data['status']==1) {?>
                            <a class="btn btn-success" id="change_status" data-id="<?php echo $about_data['c_id']; ?>" data-admin_id="<?php echo $about_data['admin_id']; ?>" data-status="<?php echo $about_data['status']; ?>">Active</a>
                            <?php } else {?>
                  ...
                  <td><a class="btn btn-info btn-sm editproject" data-toggle="modal" data-target="#myModal" id="update" data-id="<?php echo $about_data['c_id']; ?>" data-admin_id="<?php echo $about_data['admin_id']; ?>"><span class="fa fa-pencil-square-o"></span> </a> </td>
						  
                      <td><a style="text-decoration:none;" class="approved btn btn-danger" id="delete" data-id="<?php echo $about_data['c_id']; ?>" data-admin_id="<?php echo $about_data['admin_id']; ?>" ><i class="fa fa-trash"></i> </a>
                                                      </td>
                        </tr>
...
$(document).ready(function(){
	...
	$("body").on("click","#update_category",function(){ 
	 ...
	 if($("#c_description").val()==''){
		a_response1.show().html('<div class="alert alert-danger">Please Enter Comment Description </div>'); 
		return false;
	 }
	 else{
