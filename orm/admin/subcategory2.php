  ...
  <div class="boxed">
    ...
    <div id="content-container">
      <div class="pageheader hidden-xs">
        <h3><i class="fa fa-file-text-o"></i>Sub Category 2</h3>
        <div class="breadcrumb-wrapper"> <span class="label">You are here:</span>
          <ol class="breadcrumb">
            <li> <a href="dashbord.php"> Home </a> </li>
            <li class="active">Sub Category 2</li>
          </ol>
        </div>
      </div>
      <!--Page content-->
...
$totalEmpSQL = "select * from subcategory2 where admin_id='".$admin_id."'";
...
$query = "select * from subcategory2  order by sub_cat2_id desc LIMIT $startFrom, $showRecordPerPage";
				
				//$query="Select * from subcategory2 where admin_id='".$admin_id."' order by sub_cat2_id desc";
					  $i=1;					  
                  ...
                  <div class="table-responsive">
                    <table id="demo-dt-basic" class="table table-striped table-bordered">
                      <thead>
                        <tr>				<th>Sr. No.</th>
												<th>Category NAME</th>
													<th>Sub Category NAME 1</th>
													<th>Sub Category NAME 2</th>
														<th>Meta</th>
														<th>Status</th>
															<th>EDIT</th>
																<th>DELETE</th>
                      ...
                      <tbody  id="project_search2">
						...
						<td> <?php echo $i; ?></td>
                         <?php   $query2="Select * from category where category_id='".$about_data['cat_id']."'";
							$data2=fetch_query($con,$query2);
							
							$query3="Select * from subcategory1 where sub_cat_id='".$about_data['sub_cat_id']."'";
							$data3=fetch_query($con,$query3);
							?>
							<td><?php echo $data2[0]['category_name']; ?></td>
							<td><?php echo $data3[0]['sub_cat_name']; ?></td>
                          <td><?php echo $about_data['sub_cat2_name']; ?></td>
     <td align="center"><a href="meta?S_ID=<?php echo $about_data['sub_cat2_id']; ?>&&type=subcategory2"  class="btn btn-warning"> Add Meta</a> </td>                      
                          <td><?php if($about_data['status']==1) {?>
                            <a class="btn btn-success" id="change_status" data-id="<?php echo $about_data['sub_cat2_id']; ?>" data-admin_id="<?php echo $about_data['admin_id']; ?>" data-status="<?php echo $about_data['status']; ?>">Active</a>
                            <?php } else {?>
                               <a class="btn btn-danger" id="change_status" data-id="<?php echo $about_data['sub_cat2_id']; ?>" data-admin_id="<?php echo $about_data['admin_id']; ?>" data-status="<?php echo $about_data['status']; ?>">In-active</a>
                              <?php
						}
						?></td>
                          
                  <td><a class="btn btn-info btn-sm editproject" data-toggle="modal" data-target="#myModal" id="update" data-id="<?php echo $about_data['sub_cat2_id']; ?>" data-admin_id="<?php echo $about_data['admin_id']; ?>"><span class="fa fa-pencil-square-o"></span> </a> </td>
						  
                      <td><a style="text-decoration:none;" class="approved btn btn-danger" id="delete" data-id="<?php echo $about_data['sub_cat2_id']; ?>" data-admin_id="<?php echo $about_data['admin_id']; ?>" ><i class="fa fa-trash"></i></a>
                                                      </td>
                        </tr>
...
$(document).ready(function(){
	 a_url='ajax/subcategory2.php';
	 var a_response=$(".modal-content");	
	 $("body").on("change","#cat_id",function(){
		var selval=$(this).find('option:selected').val(); 
		var a_response1=$("#sub_cat_id");
		var a_request1="action=GET_SUBCATLIST&CAT_ID="+selval+"&admin_id="+$(this).data("admin_id")+"&selected_id="+$(this).data("selected_id");
		 var redirecturl='';
		fetch_data(a_url,a_request1,a_response1,redirecturl)
	 });
	...
	$("body").on("click","#update_category",function(){ 
	  var a_response1=$("#error-subcatgory2");
	 if($("#cat_id").val()==''){
		a_response1.show().html('<div class="alert alert-danger">Please Select Category  </div>'); 
		return false;
	 }else if($("#sub_cat_id").val()==''){
		a_response1.show().html('<div class="alert alert-danger">Please Select  Sub Category </div>'); 
		return false;
	 }
	 else if($("#sub_cat2_name").val()==''){
		a_response1.show().html('<div class="alert alert-danger">Please Enter  Sub Category Name</div>'); 
		return false;
	 }
	 else{
	...
	$("body").on("click","#change_status",function(){	   
		...
		$("body").on("click","#delete",function(){	   
		   ...
		   var admin_id=$(this).data('admin_id');
		   var error_id=$("#error-msg-page");
		   var postdata="action=DELETE_DATA"+"&sub_cat2_id="+a_id+"&admin_id="+admin_id;
